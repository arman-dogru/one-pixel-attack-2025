...
class ResNet:
    ...
    def train(self):
        ...
        # Rebuild model architecture for training
        img_input = Input(shape=(self.img_rows, self.img_cols, self.img_channels))
        output    = self.residual_network(img_input, self.num_classes, self.stack_n)
        ...
        # Note: here checkpoint saves only weights if we want to reload them using load_weights
        checkpoint = ModelCheckpoint(self.model_filename, monitor='val_loss', verbose=0,
